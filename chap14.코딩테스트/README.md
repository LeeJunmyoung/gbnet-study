...
2. 개선된 다익스트라 알고리즘
    > 개선된 다익스트라의 경우 최악의 경우에도 시간 복잡도 O(ElogV)를 보장한다.  
    > V는 노드의 개수, E는 간선의 개수를 의미한다.   
...
방향성| 방향 혹은 무방향 | 방향  
순환성| 순환 및 비순환 | 비순환
루트 노드 존재 여부 | 루트노드가 없음 | 루트노드가 존재  
노드간 관계성 | 관계없음 | 부모와 자식 관계
모델의 종류 | 네트워크 | 계층
...
> 서로소 집합 자료 구조는 두 개의 유용한 연산을 제공한다:
