...
> 그래서 '길 찾기'문제라고 불린다.  
> 다익스트라, 플로이드 워셜, 벨만포드, SPFA 알고리즘이 있다.   

### 8-1. 다익스트라 알고리즘
> 다익스트라 최단 경로 알고리즘은 그래프에서 여러개의 노드가 있을때,   
> 특정한 노드에서 출발하여 다른 노드로 가는 각각의 최단 경로를 구해주는 알고리즘이다.  
> 다익스트라 알고리즘은 '음의 간선'이 없을때 정상적으로 작동한다.  
...
> ex) 1번노드에서 다른 노드로 가는 최단 경로를 구하는 문제.  

![다익스트라 알고리즘](./img/dijkstra.png)
...
* 2회전(1번노드에서 연결된지점 거리 계산)

    노드번호| 1(O) | 2 | 3 | 4 | 5| 6 |
    ---     |---|---|---|---|---|---|
    거리    | 0 | ∞(2) | ∞(5) | ∞(1) | ∞ | ∞ |

* 3회전(가장 가까운 미탐색 4번 노드로 가서 거리 계산)

    노드번호| 1(O) | 2 | 3 | 4(O) | 5| 6 |
    ---     |---|---|---|---|---|---|
    거리    | 0 | 2 | 5(4) | 1 | ∞(2) | ∞ |

* 4회전(가장 가까운 미탐색 2번 노드로 가서 거리 계산)

    노드번호| 1(O) | 2(O) | 3 | 4(O) | 5 | 6 |
    ---     |---|---|---|---|---|---|
    거리    | 0 | 2 | 4 | 1 | 2 | ∞ |

* 5회전(가장 가까운 미탐색 5번 노드로 가서 거리 계산)

    노드번호| 1(O) | 2(O) | 3 | 4(O) | 5(O) | 6 |
    ---     |---|---|---|---|---|---|
    거리    | 0 | 2 | 4(3) | 1 | 2 | ∞(4) |

* 6회전(가장 가까운 미탐색 3번 노드로 가서 거리 계산)

    노드번호| 1(O) | 2(O) | 3(O) | 4(O) | 5(O) | 6 |
    ---     |---|---|---|---|---|---|
    거리    | 0 | 2 | 3 | 1 | 2 | 4 |

* 7회전(마지막 6번 노드로 가서 거리 계산)

    노드번호| 1(O) | 2(O) | 3(O) | 4(O) | 5(O) | 6(O) |
    ---     |---|---|---|---|---|---|
    거리    | 0 | 2 | 3 | 1 | 2 | 4 |
...
1. 간단한 다익스트라 알고리즘
    ...
    > V는 노드의 개수를 의마한다.  
    > 각 노드에 대한 최단 거리를 담는 1차원리스트를 선언 후, 방문하지 않은 노드 중에서 최단 거리가 가장 짧은 노드를 선택하기 위해 매 단계마다 1차원 리스트의 모든 노드를 확인한다.(순차탐색)